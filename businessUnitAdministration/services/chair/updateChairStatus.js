...
const updateChairStatustoReady = async ({clientId,chairId,patientId,appointmentId, user})=>{
    try {
        ...
        if(selectedChair?.status  !== 'Ready' && selectedChair?.activePatientId){
           ...
           const updated = await Chairs.updateOne ({_id: new mongoose.Types.ObjectId( selectedChair?._id) },{$set:{activeAppointmentId:appointmentId,activePatientId:patientId,status:'Ready', updatedBy: user }}) 
