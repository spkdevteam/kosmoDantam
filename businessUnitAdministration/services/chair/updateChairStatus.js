...
const updateChairStatustoReady = async ({clientId,chairId,patientId,appointmentId})=>{
    try {
        ...
        else if (selectedChair?.status  !== 'Ready' && !selectedChair?.activePatientId){
            return {status:false, message:'chair is getting ready  '}
        }
        else {
           const updated = await Chairs.updateOne ({_id: new mongoose.Types.ObjectId( selectedChair?._id) },{$set:{activeAppointmentId:appointmentId,activePatientId:patientId,status:'Ready'}}) 
           console.log(updated,'aaaaaaaaaaaaaaaaaaaaaaaaa') 
...
module.exports = updateChairStatustoReady