...
const CancelChairReadyStatus = async ({ clientId, chairId, patientId, appointmentId, user }) => {
    try {
        ...
        const updated = await Chairs.updateOne({ _id: new mongoose.Types.ObjectId(selectedChair?._id) }, { $set: { activeAppointmentId:null ,activePatientId: null, status: 'Ready', updatedBy: user } });
