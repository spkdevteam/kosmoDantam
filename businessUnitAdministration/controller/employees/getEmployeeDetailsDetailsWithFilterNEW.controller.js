...
const getEmployeeDetailsDetailsWithFilterFnNEW = require("../../services/employees/getEmployeeDetailsDetailsWithFilterFnNEW");

const getEmployeeDetailsDetailsWithFilterNEW = async (req, res, next) => {
    try{
        ...
        if(!businessUnit){
            return res.status(400).json({ status: false, message : "BusinessUnit is mandatory!!!!"});
        }
        const validation = [
            clientIdValidation({ clientId }),
            mongoIdValidation({ _id: businessUnit, name: "businessUnitId" })
        ];
        if (branch) {
        ...
        const result = await getEmployeeDetailsDetailsWithFilterFnNEW({ page, perPage, searchKey, fromDate, toDate, role, businessUnit, branch, createdUser, updatedUser, deletedUser, clientId });
...
module.exports = getEmployeeDetailsDetailsWithFilterNEW;